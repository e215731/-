...
# 過去6日分を特徴量、その翌日の湿度を目的変数とする訓練データを作成する関数
...
# 予測値に1.1倍の補正を加える
pre_y_2024_corrected = pre_y_2024 * 1.01
...
predicted_2024_df = pd.DataFrame({
    ...
    '月': months_2024,
    '日': days_2024,
    '予測湿度': pre_y_2024_corrected
})

# 2023年の湿度データを取得
actual_2023_df = temper_data[test_year_2023][['月', '日', '湿度']].iloc[interval:]

# 2023年と2024年の湿度データを比較するためにプロット
plt.figure(figsize=(10, 6), dpi=100)
plt.plot(actual_2023_df['湿度'].values, c='r', label='2023年の実際の湿度')
plt.plot(pre_y_2024_corrected, c='b', label='2024年の予測湿度 (補正後)')
plt.legend()
plt.xlabel('日数')
plt.ylabel('湿度 (%)')
plt.title('2023年の実際の湿度と2024年の予測湿度 (補正後) の比較')
plt.savefig('tenki-shitsudo-2023_vs_2024_corrected.png')
plt.show()

# 予測した2024年の湿度データをCSVファイルとして保存
